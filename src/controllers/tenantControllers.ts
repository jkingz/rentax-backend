...
export const getTenant = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(500).json({ message: `Failed to get tenant: ${error.message}` });
    return;
  }
};
...
export const updateTenant = async (
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: `Failed to update tenant: ${error.message}` });
    return;
  }
};
...
export const addFavoriteProperty = async (
  ...
  try {
    ...
    const existingFavorites = tenant?.favorites.find || [];
    if (
      !Array.isArray(existingFavorites) ||
      !existingFavorites.find((fav) => fav.id === propertyIdNumber)
    ) {
